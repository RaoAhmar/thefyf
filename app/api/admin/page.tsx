...
export default function AdminPage() {
  ...
  const [state, setState] = useState<"loading" | "noauth" | "forbidden" | "ok" | "error">(
    "loading"
  );
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      <p className="mt-2 opacity-70">Read-only list with Accept/Decline.</p>
      ...
      {state === "error" && (
        <div className="mt-8 rounded-2xl border p-5 text-red-400">Couldn’t load requests. Try refreshing.</div>
      )}

      {state === "ok" && rows && (
        <>
          <div className="mt-6 text-sm opacity-70">{rows.length} request{rows.length === 1 ? "" : "s"}</div>
