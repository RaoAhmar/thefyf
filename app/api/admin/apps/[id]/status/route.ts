...
type AppRow = {
  ...
  country: string | null;
  city: string | null;
  photo_url: string | null;       // <-- use this
  rate: number | null;
  tags: string[] | null;
  experience:
    | { title?: string; company?: string; start?: string; end?: string | null; current?: boolean }[]
    | null;
  status: string;
};
...
async function ensureUniqueSlug(base: string) {
  ...
  for (let i = 0; i < 6; i++) {
    const { data } = await supabaseAdmin.from("mentors").select("id").eq("slug", candidate).limit(1).maybeSingle();
...
async function approve(app: AppRow) {
  ...
  // Upsert by user_id manually
  const existing = await supabaseAdmin.from("mentors").select("id").eq("user_id", app.user_id).maybeSingle();
  ...
  if (existing.data) {
    return await supabaseAdmin
      ...
      .update({
        ...
        location,
        years_exp: years,
        photo_url: app.photo_url ?? null,   // <-- NEW
      })
...
async function setApplicationStatus(id: string, status: string) {
  return await supabaseAdmin.from("mentor_applications").update({ status }).eq("id", id).select("id,status").single();
}

async function handleAction(id: string, action: Action) {
  const { data: app, error: loadErr } = await supabaseAdmin.from("mentor_applications").select("*").eq("id", id).single();
  if (loadErr || !app) {
  ...
  const map: Record<Action, string> = { approve: "approved", decline: "declined", suspend: "suspended", block: "blocked" };
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!action || !["approve", "decline", "suspend", "block"].includes(action)) return json({ ok: false, error: "invalid_action" }, 400);
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (ct.includes("application/json")) {
    try { action = ((await req.json()) as { action?: Action }).action; } catch { return json({ ok: false, error: "bad_json" }, 400); }
  } else {
